...
MCL0_p_exp.2_gpt <- function(grid, Z, center, border, core){
  ...
  ############################################################
  # Pre-compute GpGp ordering and nearest neighbors
  #
  # IMPORTANT:
  # order_maxmin() contains rnorm() and sample(), so it is
  # computed ONCE before parallel computation.
  #
  # The same ord and NNar will then be used in Step 1 and Step 2.
  ############################################################

  set.seed(0)

  ord_list <- vector("list", K)
  NN_list <- vector("list", K)
  ...
  for(v in 1:K){
    ...
    # Vecchia nearest-neighbor structure
    NNar <- GpGp::find_ordered_nn(
      locsord,
      50
    )
  ...
  R1 <- foreach(
    ...
    # Use pre-computed Vecchia neighbors
    NNar <- NN_list[[v]]
  ...
  R2 <- foreach(
    ...
    # Use EXACTLY the same ordering as Step 1
    ord <- ord_list[[v]]

    locsord <- locs[ord,]
    Dord <- D[ord,]

    # Use EXACTLY the same NN structure as Step 1
    NNar <- NN_list[[v]]
  ...
  sigmasqvector_hat <- matrix(
    R2[1,],
    K,
    1
  )

  alphavector_hat <- matrix(
    R2[2,],
    K,
    1
  )
  ...
  # Set seed separately for the random sample used in Step 3
  set.seed(0)
...
MCL0_exp.2_gpt <- function(grid, Z, center, border, core = NULL){
  ...
  # Pre-compute GpGp ordering and nearest neighbors
  #
  # The same ord and NNar are used in Step 1 and Step 2.
  ############################################################
  ...
  cat("R1[1,v] =", format(R1[1,v], digits = 17), "\n")
  cat("R1[2,v] =", format(R1[2,v], digits = 17), "\n")
  cat("R1[3,v] =", format(R1[3,v], digits = 17), "\n")
  cat("tau_hat  =", format(tau_hat, digits = 17), "\n")

  cat("initial sigma2 =",
      format(R1[1,v] + R1[3,v] - tau_hat, digits = 17), "\n")

  cat("initial alpha =",
      format(R1[2,v], digits = 17), "\n")
  ############################################################
  # Step 2
  ############################################################
  ...
  for(v in 1:K){
    ...
    # EXACTLY the same ordering as Step 1
    ord <- ord_list[[v]]
    
    locsord <- locs[ord,]
    Dord <- D[ord,]
    
    # EXACTLY the same NN structure as Step 1
    NNar <- NN_list[[v]]
  ...
  # Set seed separately for random sample used in Step 3
  set.seed(0)
