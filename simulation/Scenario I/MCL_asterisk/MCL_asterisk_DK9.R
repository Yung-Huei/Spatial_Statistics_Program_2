...
T <- 100
N <- 1000
pred_DK9 <- matrix(,T,g^2)
SPEu_DK9 <- matrix(,T,g^2)
mspe_DK9 <- matrix(,T,1)
timeDK9 <- matrix(,T,3)
...
for( u in 1:T ){
  ...
  DK9 <- DK.sim(E=ER, center_p=center_p, grids=grids)

  timeDK9[u,] <- (proc.time()-t1)[1:3]
  pred_DK9[u,] <- DK9$predictor
  mspe_DK9[u,1] <- mean((pred_DK9[u,]-y)^2)
  SPEu_DK9[u,] <- (pred_DK9[u,]-y)^2
  save.image("MCL_DK9.RData")
}

round(mean(mspe_DK9[,1]),4)
round(sd(mspe_DK9[,1])/sqrt(T),4)
round(sapply(1:3, function(j){mean(timeDK9[1:T,j])}),4)
round(sapply(1:3, function(j){sd(timeDK9[1:T,j])/sqrt(T)}),4)
