...
T <- 100
N <- 1000
pred_DK16 <- matrix(,T,g^2)
SPEu_DK16 <- matrix(,T,g^2)
mspe_DK16 <- matrix(,T,1)
timeDK16 <- matrix(,T,3)
...
for( u in 1:T ){
  ...
  DK16 <- DK.sim(E=ER, center_p=center_p, grids=grids)

  timeDK16[u,] <- (proc.time()-t1)[1:3]
  pred_DK16[u,] <- DK16$predictor
  mspe_DK16[u,1] <- mean((pred_DK16[u,]-y)^2)
  SPEu_DK16[u,] <- (pred_DK16[u,]-y)^2
  save.image("MCL_DK16.RData")
}

round(mean(mspe_DK16[,1]),4)
round(sd(mspe_DK16[,1])/sqrt(T),4)
round(sapply(1:3, function(j){mean(timeDK16[1:T,j])}),4)
round(sapply(1:3, function(j){sd(timeDK16[1:T,j])/sqrt(T)}),4)
