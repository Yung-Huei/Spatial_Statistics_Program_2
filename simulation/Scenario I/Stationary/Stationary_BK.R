...
for( u in 1:T ){
  ...
  Sigma_yii <- lapply(1:K, function (k) {
                 d <- dist1(matrix(grid[Q[1,k]:Q[2,k],],q[k],2))
                 error <- diag(paraML[u,3], q[k])
                 covariance <- paraML[u,1]*matern(d, 1/paraML[u,2], 0.5)+ error                        
               } )
  ...
  pcov <- lapply(1:K, function(i){
            ...
            if(length(o)==0){
              ...
              d <- dist2(matrix(grids[o,],length(o),ncol(grids)), matrix(grid[Q[1,i]:Q[2,i],],q[i],ncol(grids)))
              return( paraML[u,1]*matern(d, 1/paraML[u,2], 0.5) )
            }
          })            
