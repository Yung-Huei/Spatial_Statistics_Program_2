...
for( u in 1:T ){
  ...
  coords2 <- data.frame(x=grids[sample400,1],y=grids[sample400,2])
  t1 <- proc.time()
  center <- as.matrix(spatdiv(coor=coords2,data=Z,k=4,area=1)$center$coor)
  steptime[u,5] <- (proc.time()-t1)[1]
  center <- center[order(center[,2]),]
  REP[[u]] <- center
  ...
  save.image("simulationPAR.RData")
  if(u==10| u==50){
  print(round(sapply(1:(3*K+4), function(j){mean(para[1:u,j])}),4))
  print(round(sapply(1:(3*K+4), function(j){sd(para[1:u,j])/sqrt(u)}),4))
  print(round(sapply(1:5, function(j){mean(steptime[1:u,j])}),4))
  print(round(sapply(1:5, function(j){sd(steptime[1:u,j])/sqrt(u)}),4))
...
round(sapply(1:(3*K+4), function(j){sd(para[1:T,j])/sqrt(T)}),4)
round(sapply(1:5, function(j){mean(steptime[1:T,j])}),4)
round(sapply(1:5, function(j){sd(steptime[1:T,j])/sqrt(u)}),4)
round(mean(F_loss[,1]),4)
round(sd(F_loss[,1])/sqrt(T),4)
round(mean(KL_loss[,1]),4)
round(sd(KL_loss[,1])/sqrt(T),4)
