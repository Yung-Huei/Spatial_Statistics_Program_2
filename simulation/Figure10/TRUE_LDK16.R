...
pred_LDK16[u,] <- sapply( 1:predictsize, function (k) {
...
save.image("TRUE_LDK16.RData")
}

round(mean(mspe_LDK16[,1]),4)
round(sd(mspe_LDK16[,1])/sqrt(T),4)
round(mean(timeLDK16[,1]),4)
round(sd(timeLDK16[,1])/sqrt(T),4)
write.csv(mspe_LDK16, "LDK16.csv", row.names=FALSE)
write.csv(timeLDK16, "timeLDK16.csv", row.names=FALSE)
