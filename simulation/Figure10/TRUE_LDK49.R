...
pred_LDK49 <- matrix(,T,g^2)
SPEu_LDK49 <- matrix(,T,g^2)
mspe_LDK49 <- matrix(,T,1)
timeLDK49 <- matrix(,T,3)

sp = seq(1/2/7, 1-1/2/7, l = 7)
...
pred_LDK49[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK49[u,] <- (proc.time()-t1)[1:3]
mspe_LDK49[u,1] <- mean((pred_LDK49[u,]-y)^2)
SPEu_LDK49[u,] <- (pred_LDK49[u,]-y)^2
save.image("TRUE_LDK49.RData")
}

round(mean(mspe_LDK49[,1]),4)
round(sd(mspe_LDK49[,1])/sqrt(T),4)
round(mean(timeLDK49[,1]),4)
round(sd(timeLDK49[,1])/sqrt(T),4)
write.csv(mspe_LDK49, "LDK49.csv", row.names=FALSE)
write.csv(timeLDK49, "timeLDK49.csv", row.names=FALSE)
