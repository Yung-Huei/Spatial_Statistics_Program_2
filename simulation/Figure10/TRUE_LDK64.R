...
pred_LDK64 <- matrix(,T,g^2)
SPEu_LDK64 <- matrix(,T,g^2)
mspe_LDK64 <- matrix(,T,1)
timeLDK64 <- matrix(,T,3)

sp = seq(1/2/8, 1-1/2/8, l = 8)
...
pred_LDK64[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK64[u,] <- (proc.time()-t1)[1:3]
mspe_LDK64[u,1] <- mean((pred_LDK64[u,]-y)^2)
SPEu_LDK64[u,] <- (pred_LDK64[u,]-y)^2
save.image("TRUE_LDK64.RData")
}

round(mean(mspe_LDK64[,1]),4)
round(sd(mspe_LDK64[,1])/sqrt(T),4)
round(mean(timeLDK64[,1]),4)
round(sd(timeLDK64[,1])/sqrt(T),4)
write.csv(mspe_LDK64, "LDK64.csv", row.names=FALSE)
write.csv(timeLDK64, "timeLDK64.csv", row.names=FALSE)
