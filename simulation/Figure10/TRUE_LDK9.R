...
pred_LDK9 <- matrix(,T,g^2)
SPEu_LDK9 <- matrix(,T,g^2)
mspe_LDK9 <- matrix(,T,1)
timeLDK9 <- matrix(,T,3)

sp = seq(1/2/3, 1-1/2/3, l = 3)
...
pred_LDK9[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK9[u,] <- (proc.time()-t1)[1:3]
mspe_LDK9[u,1] <- mean((pred_LDK9[u,]-y)^2)
SPEu_LDK9[u,] <- (pred_LDK9[u,]-y)^2
save.image("TRUE_LDK9.RData")
}

round(mean(mspe_LDK9[,1]),4)
round(sd(mspe_LDK9[,1])/sqrt(T),4)
round(mean(timeLDK9[,1]),4)
round(sd(timeLDK9[,1])/sqrt(T),4)
write.csv(mspe_LDK9, "LDK9.csv", row.names=FALSE)
write.csv(timeLDK9, "timeLDK9.csv", row.names=FALSE)
