...
library(ggvoronoi);
library(deldir);library(ggplot2);library(SpatialTools);library(cluster)
...
pred_LDK100 <- matrix(,T,g^2)
SPEu_LDK100 <- matrix(,T,g^2)
mspe_LDK100 <- matrix(,T,1)
timeLDK100 <- matrix(,T,3)

sp = seq(1/2/10, 1-1/2/10, l = 10)
...
Sigma_yii <- lapply(1:M, function (k) {
                                       ...
                                       covariance <- cov.fun(matrix(grid_p[Qmatrix_p[1,k]:Qmatrix_p[2,k],],qq[k],2), 
neighbormatrix, 
matrix(grid_segment_p[Qmatrix_p[1,k]:Qmatrix_p[2,k],],qq[k],ncol(grid_segment_p)), 
matrix(grid_region_p[Qmatrix_p[1,k]:Qmatrix_p[2,k],],qq[k],ncol(grid_region_p)), 
...
pred_LDK100[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK100[u,] <- (proc.time()-t1)[1:3]
mspe_LDK100[u,1] <- mean((pred_LDK100[u,]-y)^2)
SPEu_LDK100[u,] <- (pred_LDK100[u,]-y)^2
save.image("TRUE_LDK100.RData")
}

round(mean(mspe_LDK100[,1]),4)
round(sd(mspe_LDK100[,1])/sqrt(T),4)
round(mean(timeLDK100[,1]),4)
round(sd(timeLDK100[,1])/sqrt(T),4)
write.csv(mspe_LDK100, "LDK100.csv", row.names=FALSE)
write.csv(timeLDK100, "timeLDK100.csv", row.names=FALSE)
