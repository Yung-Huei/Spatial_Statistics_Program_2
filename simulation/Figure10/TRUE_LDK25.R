...
pred_LDK25 <- matrix(,T,g^2)
SPEu_LDK25 <- matrix(,T,g^2)
mspe_LDK25 <- matrix(,T,1)
timeLDK25 <- matrix(,T,3)

sp = seq(1/2/5, 1-1/2/5, l = 5)
...
pred_LDK25[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK25[u,] <- (proc.time()-t1)[1:3]
mspe_LDK25[u,1] <- mean((pred_LDK25[u,]-y)^2)
SPEu_LDK25[u,] <- (pred_LDK25[u,]-y)^2
save.image("TRUE_LDK25.RData")
}

round(mean(mspe_LDK25[,1]),4)
round(sd(mspe_LDK25[,1])/sqrt(T),4)
round(mean(timeLDK25[,1]),4)
round(sd(timeLDK25[,1])/sqrt(T),4)
write.csv(mspe_LDK25, "LDK25.csv", row.names=FALSE)
write.csv(timeLDK25, "timeLDK25.csv", row.names=FALSE)
