...
pred_LDK36 <- matrix(,T,g^2)
SPEu_LDK36 <- matrix(,T,g^2)
mspe_LDK36 <- matrix(,T,1)
timeLDK36 <- matrix(,T,3)

sp = seq(1/2/6, 1-1/2/6, l = 6)
...
pred_LDK36[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK36[u,] <- (proc.time()-t1)[1:3]
mspe_LDK36[u,1] <- mean((pred_LDK36[u,]-y)^2)
SPEu_LDK36[u,] <- (pred_LDK36[u,]-y)^2
save.image("TRUE_LDK36.RData")
}

round(mean(mspe_LDK36[,1]),4)
round(sd(mspe_LDK36[,1])/sqrt(T),4)
round(mean(timeLDK36[,1]),4)
round(sd(timeLDK36[,1])/sqrt(T),4)
write.csv(mspe_LDK36, "LDK36.csv", row.names=FALSE)
write.csv(timeLDK36, "timeLDK36.csv", row.names=FALSE)
