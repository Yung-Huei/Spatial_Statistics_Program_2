...
pred_LDK4 <- matrix(,T,g^2)
SPEu_LDK4 <- matrix(,T,g^2)
mspe_LDK4 <- matrix(,T,1)
timeLDK4 <- matrix(,T,3)

sp = seq(1/2/2, 1-1/2/2, l = 2)
...
pred_LDK4[u,] <- sapply( 1:predictsize, function (k) {
...
timeLDK4[u,] <- (proc.time()-t1)[1:3]
mspe_LDK4[u,1] <- mean((pred_LDK4[u,]-y)^2)
SPEu_LDK4[u,] <- (pred_LDK4[u,]-y)^2
save.image("TRUE_LDK4.RData")
}

round(mean(mspe_LDK4[,1]),4)
round(sd(mspe_LDK4[,1])/sqrt(T),4)
round(mean(timeLDK4[,1]),4)
round(sd(timeLDK4[,1])/sqrt(T),4)
write.csv(mspe_LDK4, "LDK4.csv", row.names=FALSE)
write.csv(timeLDK4, "timeLDK4.csv", row.names=FALSE)
