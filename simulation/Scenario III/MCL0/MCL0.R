...
for( u in 1:T ){
  ...
  save.image("simulationMCL0.RData")
  if(u==10| u==50){
  print(round(sapply(1:(3*K+4), function(j){mean(para[1:u,j])}),4))
  print(round(sapply(1:(3*K+4), function(j){sd(para[1:u,j])/sqrt(u)}),4))
  print(round(sapply(1:3, function(j){mean(steptime[1:u,j])}),4))
  print(round(sapply(1:3, function(j){sd(steptime[1:u,j])/sqrt(u)}),4))
...
round(sapply(1:3, function(j){mean(steptime[1:T,j])}),4)
round(sapply(1:3, function(j){sd(steptime[1:T,j])/sqrt(T)}),4)
